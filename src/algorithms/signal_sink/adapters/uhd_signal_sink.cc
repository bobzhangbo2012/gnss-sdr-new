...
UhdSignalSink::UhdSignalSink(ConfigurationInterface* configuration,
    ...
    std::shared_ptr<Concurrent_Queue<pmt::pmt_t>> queue) : role_(role), in_stream_(in_stream), out_stream_(out_stream), queue_(std::move(queue))
{
    ...
    length_tag_ = configuration->property(role + ".length_tag", default_length_tag);
    ...
    //uhd_sink_= uhd::usrp::multi_usrp::make(dev_addr);
    ...
    uhd_sink_ = gr::uhd::usrp_sink::make(dev_addr, uhd_stream_args_, length_tag_);
    ...
    uhd_sink_->set_subdev_spec(subdevice_, 0);

    // 2.1 set sampling clock reference
    // Set the clock source for the usrp device.
    // Options: internal, external, or MIMO
    uhd_sink_->set_clock_source(clock_source_);

    // 2.2 set the sample rate for the usrp device
    uhd_sink_->set_samp_rate(sample_rate_);
    // the actual sample rate may differ from the rate set
    std::cout << boost::format("Sampling Rate for the USRP device: %f [sps]...") % (uhd_sink_->get_samp_rate()) << std::endl;
    LOG(INFO) << boost::format("Sampling Rate for the USRP device: %f [sps]...") % (uhd_sink_->get_samp_rate());
    ...
    for (int i = 0; i < RF_channels_; i++)
        {
            ...
            uhd_sink_->set_center_freq(freq_.at(i), i);
            std::cout << boost::format("Actual USRP center freq.: %f [Hz]...") % (uhd_sink_->get_center_freq(i)) << std::endl;
            LOG(INFO) << boost::format("Actual USRP center freq. set to: %f [Hz]...") % (uhd_sink_->get_center_freq(i));

            // TODO: Assign the remnant IF from the PLL tune error
            std::cout << boost::format("PLL Frequency tune error %f [Hz]...") % (uhd_sink_->get_center_freq(i) - freq_.at(i)) << std::endl;
            LOG(INFO) << boost::format("PLL Frequency tune error %f [Hz]...") % (uhd_sink_->get_center_freq(i) - freq_.at(i));

            // 4. set the gain for the daughterboard
            uhd_sink_->set_gain(gain_.at(i), i);
            std::cout << boost::format("Actual daughterboard gain set to: %f dB...") % uhd_sink_->get_gain(i) << std::endl;
            LOG(INFO) << boost::format("Actual daughterboard gain set to: %f dB...") % uhd_sink_->get_gain(i);
            ...
            uhd_sink_->set_bandwidth(IF_bandwidth_hz_.at(i), i);
            ...
            sensor_names = uhd_sink_->get_sensor_names(i);
            if (std::find(sensor_names.begin(), sensor_names.end(), "lo_locked") != sensor_names.end())
                {
                    uhd::sensor_value_t lo_locked = uhd_sink_->get_sensor("lo_locked", i);
...
// UhdSignalSink::~UhdSignalSink() = default;


void UhdSignalSink::connect(gr::top_block_sptr top_block)
{
    for (int i = 0; i < RF_channels_; i++)
        {
            if (samples_.at(i) != 0ULL)
                {
                    top_block->connect(uhd_sink_, i, valve_.at(i), 0);
            ...
            else
                {
                    if (dump_.at(i))
                        {
                            top_block->connect(uhd_sink_, i, file_sink_.at(i), 0);
...
void UhdSignalSink::disconnect(gr::top_block_sptr top_block)
{
    for (int i = 0; i < RF_channels_; i++)
        {
            if (samples_.at(i) != 0ULL)
                {
                    top_block->disconnect(uhd_sink_, i, valve_.at(i), 0);
            ...
            else
                {
                    if (dump_.at(i))
                        {
                            top_block->disconnect(uhd_sink_, i, file_sink_.at(i), 0);
                        }
                }
        }
}
...
gr::basic_block_sptr UhdSignalSink::get_left_block()
{
    LOG(WARNING) << "Trying to get signal source left block.";
    //return gr_basic_block_sptr();
    return gr::uhd::usrp_sink::sptr();
}


gr::basic_block_sptr UhdSignalSink::get_right_block()
{
    return get_right_block(0);
}


gr::basic_block_sptr UhdSignalSink::get_right_block(int RF_channel)
{
